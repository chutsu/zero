...
function print_matrix(prefix, matrix) {
  if (prefix === undefined) prefix = "";
  if (matrix === undefined) data = [];

  var idx = 0;
  console.log("%s:\n", prefix);
  for (var i = 0; i < m; i++) {
    for (var j = 0; j < n; j++) {
      console.log("%f\t", data[idx]);
      idx++;
    }
    console.log("\n");
  }
...
function print_vector(prefix, data) {
  if (prefix === undefined) prefix = "";
  if (matrix === NULL) data = [];
...
function test_mat_new() {
  m = mat_new(2, 2);
  console.assert(m.rows == 2);
  console.assert(m.cols == 2);
}
